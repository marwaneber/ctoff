...
const modules = [
  ...
  {
    id: "ui-shell",
    name: "UI Shell (Tailwind + multilingual)",
    price: 1200,
    essential: true,
  },
...
const bonusOption = {
  id: "figma",
  name: "Figma First design option",
  price: 600,
};
...
export function ModularBuffetSection() {
  ...
  const maxCost =
    modules.reduce((sum, m) => sum + m.price, 0) + bonusOption.price;
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, margin: "-100px" });
  const [selectedModules, setSelectedModules] = useState<string[]>(
    modules.filter((m) => m.essential).map((m) => m.id),
  );
  ...
  const getPricingTier = (total: number) => {
    ...
    if (total <= 5000)
      return { name: "Professional MVP", color: "text-purple-600" };
  ...
  const handleSuccess = () => {
    ...
    setSelectedModules(modules.filter((m) => m.essential).map((m) => m.id));
    setIncludeFigma(false);
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
            Most devs tell you what they'll build. I let you choose what you
            need — and deliver it with precision.
          </p>
        </motion.div>
        ...
        <div className="max-w-4xl mx-auto">
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      <Checkbox
                        ...
                        onCheckedChange={() =>
                          handleModuleToggle(module.id, module.essential)
                        }
                  ...
                  <div className="flex items-center space-x-3">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        setIncludeFigma(checked as boolean)
                      }
                    />
                    <div className="flex items-center space-x-2">
                      <Sparkles className="w-4 h-4 text-primary" />
                      <span className="font-medium">
                        🎁 Bonus: {bonusOption.name}
                      </span>
                    </div>
                  </div>
                  <span className="font-bold text-primary">
                    ${bonusOption.price.toLocaleString()}
                  </span>
                </motion.div>
              </CardContent>
            </Card>
            ...
            <Card className="border-2 border-primary/50 shadow-2xl bg-gradient-to-br from-primary/5 to-purple-600/5">
              <CardContent className="p-8">
                ...
                <p className="text-muted-foreground mb-6">
                  💸 Starting at ${minCost} — All the way up to ${maxCost} for
                  full branded MVPs.
                </p>
                <div className="flex flex-col items-center justify-center">
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => setShowStickySummary(true)}
                    className="mt-4"
                  >
          ...
          {showSuccessMessage && (
            ...
            >
              <Card className="border-2 border-green-500/50 bg-green-50 dark:bg-green-900/20">
